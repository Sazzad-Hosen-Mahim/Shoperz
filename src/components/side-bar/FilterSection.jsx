  ...
  const handleAvailabilityChange = (type) => {
    setFilters((prevFilters) => ({
      ...
      availability: {
        ...
        [type]: !prevFilters.availability[type],
      },
    }));
  };
  const handlePriceChange = (event) => {
    const max = Number(event.target.value); // Only update max value
  
    setFilters((prevFilters) => ({
      ...prevFilters,
      priceRange: { ...prevFilters.priceRange, max }, // Keep min fixed, only update max
    }));
  };
      ...
      <div className="bg-white px-4">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            min="0"
            max="395"
            value={filters.priceRange.max} // Control only max value
            step="1"
            onChange={handlePriceChange} // Using your existing function
            className="w-full cursor-pointer"
          />
          <div className="flex justify-between text-sm mt-2">
            <span>$0</span> {/* Fixed min price */}
            <span>${filters.priceRange.max}</span> {/* Dynamic max price */}
          </div>
        </div>
      </div>
    </div>
