...
const testimonials = [
    {
        id: 1,
        text: "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Similique doloribus quidem earum...",
        name: "Emmett Torphy",
...
export default function OfferProductReview() {
    ...
    // Handle responsiveness based on screen width
    useEffect(() => {
        const updateCardsPerView = () => {
            if (window.innerWidth < 768) {
                setCardsPerView(1); // Mobile: Show 1 card
            } else {
                setCardsPerView(3); // Desktop: Show 3 cards
            }
        };
    ...
    const handleNext = () => {
        setActiveIndex((prev) => Math.min(prev + 1, testimonials.length - cardsPerView));
    };

    return (
        <div className="p-8 mt-5 w-full max-w-[1880px] mx-auto overflow-hidden">
            <div className="space-y-6">
                ...
                >
                    {testimonials.map((testimonial) => (
                        <Card
                            key={testimonial.id}
                            className="p-6 transition-all duration-300 w-full sm:w-1/3"
                            style={{
                ...
                <div className="flex justify-end gap-2 mt-6">
                    ...
                    <Button
                        variant="outline"
                        onClick={handleNext}
                        className={`${activeIndex >= testimonials.length - cardsPerView ? "opacity-50 cursor-not-allowed" : "bg-white text-black"
                            }`}
